...
export function TimelinePanel({
  ...
  return (
    <div className="relative flex h-full w-full flex-col overflow-y-auto p-6" ref={scrollContainerRef}>
      <div className="sticky top-0 z-20 mb-6 flex flex-col gap-1 bg-[#fdfbf7]/85 py-2 backdrop-blur-md">
        <h2 className="flex items-center gap-2 font-display text-lg font-medium tracking-tight text-[#2c2825]">
          <Clock className="h-4 w-4 text-[#3d3833]" />
          Itinerary
        </h2>
        <p className="font-mono text-[10px] uppercase tracking-widest text-[#5c554d]">
          Bespoke day flow
        </p>
      </div>

      <div className="relative flex-1 mt-2">
        <div className="absolute bottom-2 left-[15px] top-2 w-px bg-gradient-to-b from-[#b09e80]/50 via-[#b09e80]/35 to-[#b09e80]/10"></div>
              ...
              >
                ...
                <div
                  className={cn(
                    ...
                    item.status === "done"
                      ? "border-[#e3dcd1]/45 text-[#3d3833]"
                      : item.status === "active"
                        ? "border-[#e3dcd1] text-[#2c2825]"
                        : "border-[#e3dcd1]/35 text-[#5c554d]",
                  )}
                >
                  {item.status === "done" ? (
                ...
                <div
                  className={cn(
                    ...
                    item.status === "active"
                      ? "scale-[1.02] border-[#e3dcd1]/55 bg-white shadow-[0_4px_20px_rgba(44,40,37,0.06)]"
                      : hoveredItemId === item.id
                        ? "scale-[1.01] border-[#e3dcd1]/60 bg-white shadow-[0_4px_20px_rgba(44,40,37,0.06)]"
                        : "border-[#e3dcd1]/30 bg-white/75",
                  )}
                  onMouseEnter={() => onItemHover(item.id)}
                  onMouseLeave={() => onItemHover(null)}
                  onClick={() => onItemClick(item.id)}
                >
                  <div className="mb-1 flex items-center justify-between font-mono text-[10px] text-[#5c554d]">
                    <span>{item.time}</span>
                    {item.status === "active" && (
                      <span className="flex animate-pulse items-center gap-1 text-[#3d3833]">
                        <Circle
                          fill="currentColor"
                          strokeWidth={0}
                  ...
                  {item.status === "active" ? (
                    ...
                    >
                      <span className="font-mono text-[9px] uppercase text-[#3d3833]">
                        In progress
                      </span>
                      <div className="flex items-center gap-3">
                        <button
                          onClick={() => onSkip(item.id)}
                          className="flex items-center gap-1 text-[9px] font-medium uppercase tracking-wider text-[#5c554d] transition-colors hover:text-[#2c2825]"
                        >
                          <X className="h-3 w-3" /> Skip
                        </button>
                        <button className="rounded-md border border-[#e3dcd1]/45 bg-white px-3 py-1 text-[10px] font-medium uppercase tracking-wider text-[#2c2825] transition-colors hover:bg-white">
                          Details &rarr;
                        </button>
                      </div>
                    </motion.div>
